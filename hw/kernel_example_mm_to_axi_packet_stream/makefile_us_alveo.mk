...
HLS_DIR := ./hls
$(BUILD_DIR)/mm2s_packet.xo: $(HLS_DIR)/mm2s_packet.cpp
	mkdir -p $(BUILD_DIR)
	v++ -c -t $(TARGET) --platform $(PLATFORM) -k mm2s_packet -o $@ $<
$(BUILD_DIR)/kernel.xclbin: $(BUILD_DIR)/kernel.xo $(BUILD_DIR)/mm2s_packet.xo
	mkdir -p $(BUILD_DIR)
	v++ -l -t $(TARGET) --platform $(PLATFORM) --config u50.cfg $(VPPFLAGS) \
	    $(BUILD_DIR)/kernel.xo $(BUILD_DIR)/mm2s_packet.xo -o $@
	@if [ "$(TARGET)" = "hw" ]; then \
